...
<div id="bodyWrap">
  ...
  //$tbl_name="event";    //your table name 
    ...
    */ 
    $query = "SELECT COUNT(*) as num FROM event e, categories c, likes l where l.email='$email_id' and l.Category_id=e.category_id and e.category_id=c.category_id and e.event_type='public'"; 
    $total_pages = mysql_fetch_array(mysql_query($query)); 
    $total_pages = $total_pages[0]; 
     
    /* Setup vars for query. */ 
    $targetpage = "myinterests.php";     //your file name  (the name of this file) 
    ...
    /* Get data. */ 
    $sql = "SELECT e.event_name, c.category_name, e.description, e.event_id FROM event e, categories c, likes l where l.email='$email_id' and l.Category_id=e.category_id and e.category_id=c.category_id and e.event_type='public' LIMIT $start, $limit"; 
                        ...
                        if($con)
                          {
                            ...
                            //$email_id = $_SESSION['email'];
                            //$result = mysql_query("SELECT e.event_name, c.category_name, e.description, e.event_id FROM event e, categories c, likes l where l.email='$email_id' and l.Category_id=e.category_id and e.category_id=c.category_id and e.event_type='public'") or die ("Error in query: ".mysql_error());
                            if (mysql_num_rows($result) == 0)
                              ...
                              while($row = mysql_fetch_array($result))
                                ...
                                if($count == 0)
                                  {
                                    echo '<div class="wid980 hgt25"><p class="bold font20 fontOrange"><a href="myinterestsdisplay.php?eventId=' . $row[3] . '">' .$row[0] .'</a></p></div>';
                                    echo "<div class='wid980 hgt25'><p class='font15 fontColor'>Category Type: " . $row[1] . "</p></div>";
