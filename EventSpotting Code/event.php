...
<div id="bodyWrap">
  ...
  //$tbl_name="event";    //your table name 
    ...
    */ 
    $query = "SELECT COUNT(*) as num FROM event as e,categories as c WHERE e.category_id = c.category_id and e.date >= now() and e.event_type='public' and e.event_approval_flag='1'"; 
    $total_pages = mysql_fetch_array(mysql_query($query)); 
    $total_pages = $total_pages[0]; 
     
    /* Setup vars for query. */ 
    $targetpage = "event.php";     //your file name  (the name of this file) 
    ...
    /* Get data. */ 
    $sql = "SELECT e.event_name, c.category_name, e.description, e.event_id FROM event e, categories c where e.category_id = c.category_id and e.date >= now() and e.event_type='public' and e.event_approval_flag='1' LIMIT $start, $limit"; 
                        ...
                        if($con)
                          {
                           ...
                           // $result = mysql_query("SELECT e.event_name, c.category_name, e.description, e.event_id FROM event e, categories c where e.category_id = c.category_id and e.date >= now() and e.event_type='public' LIMIT $start, $limit") or die ("Error in query: ".mysql_error());
