...
<div id="bodyWrap">
          ...
          <div id="mainnavWrap" >
            ...
            <div class="mid dbfl">
              <ul id="nav">
                ...
                <li class="menu1"><a href="pastevent.php" class="current1" id="u2"> Past Events</a></li>
                <?php
  ...
  //$tbl_name="event";    //your table name 
    ...
    */ 
    $query = "SELECT COUNT(*) as num FROM event as e,categories as c WHERE e.category_id = c.category_id and e.date <= now() and e.event_type='public'"; 
    $total_pages = mysql_fetch_array(mysql_query($query)); 
    $total_pages = $total_pages[0]; 
     
    /* Setup vars for query. */ 
    $targetpage = "pastevent.php";     //your file name  (the name of this file) 
    ...
    /* Get data. */ 
    $sql = "SELECT e.event_name, c.category_name, e.description, e.event_id FROM event e, categories c where e.category_id = c.category_id and e.date<=now() and e.event_type='public' LIMIT $start, $limit"; 
                        ...
                        if($con)
                          {
                           ...
                           // $result = mysql_query("SELECT e.event_name, c.category_name, e.description, e.event_id FROM event e, categories c where e.category_id = c.category_id and e.event_type='public' and e.date<=now()") or die ("Error in query: ".mysql_error());
                            if (mysql_num_rows($result) == 0)
                              ...
                              while($row = mysql_fetch_array($result))
                                ...
                                if($count == 0)
                                  {
                                    echo '<div class="wid980 hgt25"><p class="bold font20 fontOrange"><a href="pasteventdisplay.php?eventId=' . $row[3] . '">' .$row[0] .'</a></p></div>';
