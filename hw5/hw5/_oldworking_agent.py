	...
	def update_object_belief(self,mypose,objname,bobject,obsvisible):
        ...
        def most_likely_trajectory(self,mypath,objname,objpercepts):
                ...
                for (mypose,objvisible) in zip(mypath,objpercepts)[1:]:
                        ...
                        bestS = dict((s,0.0) for s in states)
                        ...
                        # this is where I have calculated the transition probability
                        ...
                        for k in Vlast.keys():
                            ...
                            #bestS = max((v,k) for (k,v) in Vlast.iteritems())[1]

                            for a in nxt_states.keys():
                            	#bestS[a] = max((v,k) for (k,v) in Vlast.iteritems())[1]
                            	#print bestS[a], a

                            	if (transition[a] < Vlast[k] * nxt_states[a]):
                            		
                            		transition[a] = Vlast[k] * nxt_states[a]
                            		#print transition[a]
                            		#print transition[bestS[a]]
                        #print bestS[a]
                        ...
                        for s in states:
                                ...
                                #according to the formula i am multiplying the transition probability also 
                                #p[s] = obsprob*Vlast[best[s]]*transition[s]
                                p[s] = obsprob*transition[s]
                                #print p[s]

                                #print "state:",s,"obsprob:",obsprob,"best:",Vlast[best[s]]
